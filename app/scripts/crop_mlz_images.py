...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Detect the passport MLZ/MRZ area and crop it out into a new output folder."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=None,
        help="Input image file or folder. Defaults to MLZ_CROP_INPUT_DIR from .env.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output folder for cropped images. Defaults to MLZ_CROP_OUTPUT_DIR from .env.",
    )
    parser.add_argument(
        "--metadata",
        type=Path,
...
def main() -> None:
    args = parse_args()
    input_path = (args.input or get_mlz_crop_input_dir()).expanduser().resolve()
    output_dir = (args.output or get_mlz_crop_output_dir()).expanduser().resolve()
